...
// Filters out administrative/non-pick rows (e.g. a member standing down
// for a round) so they can't show up disguised as a cold "team" pattern.
...
function patternCandidatePool(rows) {
  const cleanRows = rows.filter(isRealPick);
  const teams = uniq(cleanRows.map(r => r.name)).filter(Boolean);
  const perTeamThresholds = teams.flatMap(team => pointThresholdCandidates(cleanRows, team));

  const fallback = aggregate(cleanRows, 'name')
    .map(x => ({ key: `name||${x.name}`, label: `${x.name} (all bet types)`, team: x.name, picks: x.picks, wins: x.wins, success: x.success }))
    .concat(aggregate(cleanRows, 'betType')
      .map(x => ({ key: `betType||${x.name}`, label: x.name, team: null, picks: x.picks, wins: x.wins, success: x.success })));

  return comboCandidates(cleanRows)
    .concat(pointThresholdCandidates(cleanRows))
